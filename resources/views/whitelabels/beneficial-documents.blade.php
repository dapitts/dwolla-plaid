            ...
            <div class="community-tab-content-wrapper active cf">
                ...
                <div class="breadcrumb-wrapper cf">
                    <ul class="breadcrumbs">
                        ...
                        <li class="active-page">Beneficial Owner Documents</li>
                    </ul>
                </div>

                <div class="r-side">
                    <div class="btn primary">
                        <a href="{{ route('wl-upload-beneficial', $beneficialOwnerId)}}" title="Upload Documents">Upload Documents</a>
                    </div>
                </div>

                <h1 class="color h1-border">Beneficial Owner Documents</h1>
                ...
                <table class="customer-docs-table">
                    ...
                    <tbody class="tbody">
                        @foreach ($beneficialDocs as $beneficialDoc)
                        <tr>
                            <td class="col-1">{{ $beneficialDoc->documentType }}</td>
                            <td class="col-2">{{ $beneficialDoc->fileName }}</td>
                            <td class="col-3">{{ $beneficialDoc->mimeType }}</td>
                            <td class="col-4">{{ $beneficialDoc->status or '' }}</td>
                            <td class="col-5">{{ $beneficialDoc->failureReason or '' }}</td>
                            @if (!empty(Auth::user()->sponsorTimezone))
                            <td class="col-6">{{ Carbon\Carbon::parse($beneficialDoc->created_at)->setTimezone(Auth::user()->sponsorTimezone)->format('n/d/Y g:i A') }}</td>
                            @else
                            <td class="col-6">{{ Carbon\Carbon::parse($beneficialDoc->created_at)->format('F j, Y') }}</td>
                            @endif
                        </tr>
                        @endforeach
                    </tbody>
