    ...
    <div class="full-pg-content-container top-padding cf">
        <div class="wrapper cf">
            ...
            <div class="community-tab-content-wrapper active cf">
                ...
                <form id="bank-acct-form" class="setting-form medium cf" action="{{ route('wl-edit-bank-acct-upd', $bankAcct->bankAccountId) }}" autocomplete="off" method="post">
                @endif
                    {{ csrf_field() }}
                    ...
                    <div id="project_accounting_details">
                        ...
                        <div class="form-section new-account-information-wrapper">
                            ...
                            <div class="label-wrapper half cf">
                                ...
                                <input type="text" class="input" value="@isset($bankAcct->wireTransferRoutingNumber){{$bankAcct->wireTransferRoutingNumber}}@endisset" placeholder="" name="wireTransferRoutingNumber" maxlength="9" id="wireTransferRoutingNumber" @if ($action == 'add' || $action == 'edit' && !$bankAcct->wireRoutingNull) readonly @endif>
                            </div>
                        </div>
...
<script>
    (async function($) {
        const fetchLinkToken = async () => {
            let response = await fetch('/whitelabel/settings/bank-account/link-token/create'),
        ...
        let config = {
            ...
            onSuccess: function(public_token, metadata) {
                $.ajax({
                    url: '/whitelabel/settings/bank-account/auth',
                    type: 'POST',
                    data: {
                    ...
                    if (auth_info.status == 'success') {
                        @if ($action == 'add')
                        $('#bank-acct-form').attr('action', location.protocol + '//' + location.hostname + '/whitelabel/settings/bank-account/edit/' + auth_info.bank_acct_id);
                        ...
                        if (auth_info.wire_routing == '')
                           $('#wireTransferRoutingNumber').attr('readonly', false);
                    } else {
