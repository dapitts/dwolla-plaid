            ...
            <div class="community-tab-content-wrapper active cf">
                <div class="breadcrumb-wrapper cf">
                    <ul class="breadcrumbs">
                        ...
                        @if ($action == 'add')
                        <li class="active-page">Add Bank Account (manual)</li>
                        @elseif ($action == 'edit')
                        <li class="active-page">Edit Bank Account (manual)</li>
                        @endif
                    </ul>
                </div>

                @if ($action == 'add')
                <h1 class="color h1-border">Add Bank Account (manual)</h1>
                @elseif ($action == 'edit')
                <h1 class="color h1-border">Edit Bank Account (manual)</h1>
                @endif

                <p>Required fields are denoted by an asterisk (*).</p>

                @if ($action == 'add')
                <form id="funding-source-form" class="setting-form medium cf" action="{{ route('wl-add-bank-acct-manual-save') }}" autocomplete="off" method="post">
                @elseif ($action == 'edit')
                <form id="funding-source-form" class="setting-form medium cf" action="{{ route('wl-edit-bank-acct-manual-upd', $bankAcct->bankAccountId) }}" autocomplete="off" method="post">
                @endif
                    {{ csrf_field() }}
                    ...
                    <div id="funding-source-info" class="cf">
                        <div id="account-info" class="lfloat pr25 rborder">
                            ...
                            <div class="label-wrapper">
                                ...
                                <input type="text" name="accountNumber" id="accountNumber" value="@isset($bankAcct->accountNumber){{$bankAcct->accountNumber}}@endisset" maxlength="17" required>
                            </div>

                            <div class="label-wrapper">
                                <span class="label-name">ABA Routing Transit Number</span>
                                <input type="text" name="abaRoutingTransitNumber" id="abaRoutingTransitNumber" value="@isset($bankAcct->abaRoutingTransitNumber){{$bankAcct->abaRoutingTransitNumber}}@endisset" maxlength="9">
                            </div>
